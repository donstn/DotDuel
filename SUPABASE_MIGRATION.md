...
2. **2-browser test** (moment of truth, **not yet run**): set `CLIENT_SUPABASE_TRANSPORT = true` in `src/types.ts`, then pair → ready → moves → clock → win → Elo in `matches`/`leaderboard`; resign; first-move abort; timeout claim; reconnect (refresh both tabs). **Watch for:** matchmake's jsonb `time_control` eq-compare (untested with real client inserts); Realtime RLS delivery needs the auth'd session on the socket (signed in before subscribe).
