...
// ===========================================================================
// clockTimeout — server-side safety net for expired clocks
// ===========================================================================
//
// Client-driven claimTimeout (App.tsx) is the primary timeout path: each
// connected client schedules a setTimeout for the exact moment the active
// clock will hit 0 and submits a `{kind:'timeout'}` action to validateMove.
//
// This sweep is the FALLBACK for the case where both clients are stuck,
// offline, asleep, or otherwise unable to fire claimTimeout — e.g.,
// hung sendMove on a privacy-strict browser leaving the game frozen with
// "both see opponent's turn" symptoms (0.1.3.0 incident). Every 1 min we
// scan active games and forfeit any whose active-player clock has expired.
//
// finalizeGame already fires on `games/{id}/status` flipping to 'finished',
// so the status flip below cascades through Elo + match-history writes
// with no further work needed here. eloFinalized guards against re-trigger.
//
// 2-second grace prevents racing with an in-flight client-driven claim.
const CLOCK_TIMEOUT_GRACE_MS = 2_000;
...
export const clockTimeout = onSchedule(
  {
  ...
  async () => {
    const rtdb = getDatabase();
    const snap = await rtdb.ref('games').get();
    if (!snap.exists()) return;
    const games = snap.val() as Record<string, ActiveGameSnapshot | null>;
    const now = Date.now();
    let forfeited = 0;
    for (const [gameId, game] of Object.entries(games)) {
      ...
      const c = game.clock;
      if (!c) continue;
      const startedAt = typeof c.turnStartedAt === 'number' ? c.turnStartedAt : 0;
      if (startedAt <= 0) continue; // game hasn't started ticking yet
      const currentSlot: 1 | 2 = c.current === 2 ? 2 : 1;
      const remaining =
        currentSlot === 1
          ? typeof c.p1RemainingMs === 'number' ? c.p1RemainingMs : 0
          : typeof c.p2RemainingMs === 'number' ? c.p2RemainingMs : 0;
      const expiresAt = startedAt + remaining;
      if (expiresAt >= now - CLOCK_TIMEOUT_GRACE_MS) continue;
      const winnerSlot: 1 | 2 = currentSlot === 1 ? 2 : 1;
      try {
        await rtdb.ref(`games/${gameId}`).update({
          status: 'finished',
          winner: winnerSlot,
          finishedReason: 'timeout',
          finishedAt: now,
        ...
        forfeited++;
        logger.info(
          `clockTimeout: forfeited ${gameId} (slot ${currentSlot} timed out, ${now - expiresAt}ms past)`,
        );
      } catch (e) {
        logger.error(`clockTimeout: forfeit write failed for ${gameId}`, e);
      }
    }
    if (forfeited > 0) {
      logger.info(`clockTimeout: forfeited ${forfeited} games this tick`);
    }
  },
);
