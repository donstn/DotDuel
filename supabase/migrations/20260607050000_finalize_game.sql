-- Phase 3: Elo finalize for a finished multiplayer game. Port of finalizeGame
-- (functions/src/index.ts). Called by the submit-move Edge Function once a game
-- reaches status='finished'. SECURITY DEFINER + the protected-write flag so it
-- can write the fn-only rating/placement columns. Idempotent via
-- matches.elo_finalized. Human-vs-human for now (both players have profiles);
-- bot Elo handled when bots land.
...
begin
  ...
  if v_ranked then
    ...
    insert into public.leaderboard (uid, display_name, rating, placement_games_played, last_played_at, is_bot)
    values (g.p1_uid, v_dn1, v_new1, v_pg1 + 1, now(), false)
    ...
    insert into public.leaderboard (uid, display_name, rating, placement_games_played, last_played_at, is_bot)
    values (g.p2_uid, v_dn2, v_new2, v_pg2 + 1, now(), false)
    on conflict (uid) do update set display_name = excluded.display_name, rating = excluded.rating,
      placement_games_played = excluded.placement_games_played, last_played_at = excluded.last_played_at;
  end if;
